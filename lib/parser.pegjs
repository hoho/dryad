...

{
  ...
  function dryadResult(first, rest) {
    var ret = [first];
...
Start
  = __ dryad:Dryad __ { return dryad; }

Dryad
  = first:DryadFunction rest:(LineTerminatorSequence+ DryadFunction)* { return dryadResult(first, rest); }

DryadFunction
  = name:DryadFunctionStart args:DryadArgumentDeclaration* ___ body:DryadFunctionBody? {
    return {
      type: "DryadFunction",
      name: name,
      args: args,
...
DryadFunctionStart
  = name:DryadFunctionName {
      dryadResetCurrentBody();
      return name;
    }

DryadFunctionName "dryad function name"
  = head:DryadFunctionNameStart tail:DryadFunctionNamePart* {
      return head + tail.join("");
    }

DryadFunctionNameStart
  = UnicodeLetter
  / "_"

DryadFunctionNamePart
  = IdentifierPart
  / "-"
...
DryadFunctionBody
  = (LineTerminatorSequence+ (DryadCommandLine / ((WhiteSpace / Comment)+)))* {
    return dryadGetCurrentBody();
  }
...
DryadJavaScriptExpression
  ...
  / "(" __ expression:Expression __ ")" { return expression; }
    ...
    return {
      type: expr.type === "ArrayExpression" ? "array" : (expr.type === "ObjectExpression" ? "object" : "expression"),
      value: text()
    };
  }
...
DryadFetch
  = "FETCH" inline:(____ DryadFetchInlinePart)* asvar:(____ "AS" DryadSpaceAndVariableName?)? {
    var inlines = {};
    var error;
    var ret;

    inline.some(function(item) {
      item = item[1];
      error = error || getErrorValue(item);
      if (item.type) {
        if (item.type in inlines) {
          error = {
            error: true,
            message: function() { return "Duplicate FETCH parameter '" + item.type + "'"; }
          };
        } else {
          inlines[item.type] = item.value;
        }
      }
      return !!error;
    });

    if (!error && asvar && !asvar[2]) {
      error = {
        error: true
      };
    }
...
DryadFetchInlinePart
  = type:("href" / "method" / "type" / "headers" / "params" / "body") value:(___ "=" ___ DryadValue?)? {
    var val = value && value[3];
    if (val) {
      return getErrorValue(val) || {type: type, value: val};
    } else {
      return {
        error: true,
        message: function() { return "Incomplete FETCH parameter '" + type + "'"; },
        location: location()
      };
    }
  }
